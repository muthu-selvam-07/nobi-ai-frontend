...
export default function ChatView({ messages, onSend, sidebarOpen }) {
  ...
  return (
    <div
      className="flex-1 mt-15 flex flex-col  transition-all duration-300 relative"
      style={{ marginLeft: sidebarOpen ? "16rem" : "0" }}
    >
      {/* Messages */}
      <div className="flex-1 overflow-y-auto  mt-35 mb-50 px-6 pt-16 pb-32 flex flex-col gap-4">
        {messages.map((msg, i) => (
          <motion.div
            key={i}
            ...
            className={`max-w-[70%] p-4 rounded-3xl break-words shadow-md ${
              msg.sender === "user"
                ? "bg-gradient-to-br from-blue-600 to-blue-500 text-white self-end ml-auto"
                : "bg-gray-800 text-gray-200 self-start mr-auto"
            }`}
          >
            {msg.image && <img src={msg.image} alt="uploaded" className="mb-2 rounded-xl shadow-sm" />}
      ...
      {/* Input */}
      <div className="absolute    bottom-15 left-1/2 transform -translate-x-1/2 w-[90%] max-w-3xl flex items-center gap-2">
        <input
          type="text"
